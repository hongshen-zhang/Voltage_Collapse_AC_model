function consider_all_mpc_uniform_reverse(mpc)
%--------------------------------------------------------------------------
% Author: Zhang HongShen
% Date : 20210512
% Plot the points figure with uniform for demand 
%--------------------------------------------------------------------------
...
for node = 1 : bus_num
    base = Binary_exact_lambda(mpc,node);
        ...
        if (mpc.bus(node,3) ~= 0 && mpc.bus(node,4)~= 0)
            if mpc.bus(node,2) == 1
                ...
                if lambda_temp >= 0.1
                    ...
                    lambda_result(count) = (base - lambda_temp)/base;
                end
            end
        end
    end
